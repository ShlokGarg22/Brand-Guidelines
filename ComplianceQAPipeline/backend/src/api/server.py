...
# Ensure we load the specific .env file from the correct location
# This matches the logic in main.py to prevent "missing env var" errors
dotenv_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), '.env')
# Fallback: if running from root, just load .env from current directory or the specific path used in main.py
if not os.path.exists(dotenv_path):
    # Try looking in ComplianceQAPipeline folder if we are at root
